<?php
	...
	if(isset($_GET['ser']))
        ...
        $count = $init->countServices();
		    $list_services = $init->listIdServices($ser);
		}elseif (isset($_POST['searcher'])) {
        $value = $_POST['searcher'];
        $count = $init->countServices();
        $list_services = $init->listSearchServices($value);
    }
    else
		{
	  	$count = $init->countServices();
      if($count == 0){
        ...
        $list_services = $init->listServices($limit1,$limit2);
  ...
  if (isset($_GET['search'])){
    ...
    $check = $init->searchService($search);
    if (isset($check) && $check != "") {
         foreach ($check as $key => $value) {
      echo '
      <a href="?page=services&ser='.$value['ServiceId'].'">
        '.$value["ServiceName"].'
        <img src="'.$value['Photo'].'" style="height:50px;width:50px">
      </a>
      <br>';
    }
    }else{
